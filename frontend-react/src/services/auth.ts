...
function loadPkce(): { verifier: string; state: string } {
  ...
  if (!verifier || !state) {
    throw new Error("Missing PKCE verifier/state (try logging in again).");
  }
...
export async function handleCallback(search: string): Promise<void> {
  ...
  const pkceData = loadPkce();
  const { verifier, state } = pkceData;
  if (!returnedState || returnedState !== state) throw new Error("State mismatch.");
  
  // Clear PKCE immediately after loading to prevent double-use
  clearPkce();
